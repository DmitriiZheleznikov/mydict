...
public class BgEffect_SlideBothSides implements IBackgroundEffect<DefaultBackground> {
    @Override
    public void appear(DefaultBackground bg, ICallable onComplete) {
        ...
        HTweener.I.to(300).onComplete(() -> {
            //init
            double xxL = - bg.getTopRect().getWidth() - bg.getShadowRadius() - 1;
            double xxR = bg.getTopRect().getWidth() + bg.getShadowRadius() + 1;
            bg.locate();
            bg.getTopRect().setX(xxR);
            bg.getTopRect().setEffect(dropShadow);

            bg.getCenterRect().setX(xxL);
            bg.getCenterRect().setEffect(dropShadow);

            bg.getBottomRect().setX(xxR);
    ...
    public void disappear(DefaultBackground bg, ICallable onComplete) {
        ...
        HTweener.I.to(300).onComplete(() -> {
            ...
            HTweener.I.to(bg.getBottomRect(), 400, Ease.quintOut).o(0.4).x(xxR).onComplete(() ->
                    HTweener.I.to(bg.getCenterRect(), 400, Ease.quintOut).o(0.4).x(xxL).onComplete(() ->
                            HTweener.I.to(bg.getTopRect(), 400, Ease.quintOut).o(0.4).x(xxR).onComplete(onComplete)
                    ));
        });
    }
}